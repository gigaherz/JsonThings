...
public class BlockSetTypeParser extends ThingParser<BlockSetType, BlockSetTypeBuilder>
{
    ...
    @Override
    public BlockSetTypeBuilder processThing(Identifier key, JsonObject data, Consumer<BlockSetTypeBuilder> builderModification)
    {
        ...
        JParse.begin(data)
                ...
                .ifKey("door_close", val -> val.string().map(Identifier::parse).handle(builder::setDoorClose))
                .ifKey("door_open", val -> val.string().map(Identifier::parse).handle(builder::setDoorOpen))
                .ifKey("trapdoor_close", val -> val.string().map(Identifier::parse).handle(builder::setTrapdoorClose))
                .ifKey("trapdoor_open", val -> val.string().map(Identifier::parse).handle(builder::setTrapdoorOpen))
                .ifKey("pressure_plate_off", val -> val.string().map(Identifier::parse).handle(builder::setPressurePlateOff))
                .ifKey("pressure_plate_on", val -> val.string().map(Identifier::parse).handle(builder::setPressurePlateOn))
                .ifKey("button_off", val -> val.string().map(Identifier::parse).handle(builder::setButtonOff))
                .ifKey("button_on", val -> val.string().map(Identifier::parse).handle(builder::setButtonOn))
                .ifKey("is_wood", val -> val.bool().handle(builder::setIsWood))
                .ifKey("hanging_sign_sound_type", val -> val.string().map(Identifier::parse).handle(builder::setHangingSignSoundType))
                .ifKey("fence_gate_close", val -> val.string().map(Identifier::parse).handle(builder::setFenceGateClose))
                .ifKey("fence_gate_open", val -> val.string().map(Identifier::parse).handle(builder::setFenceGateOpen))
