...
public class SoundTypeParser extends ThingParser<SoundTypeBuilder>
{
    ...
    private ResourceLocation fallSound;
         ...
         */
        JParse.begin(data)
                .key("volume", val -> val.floatValue().range(0,1).handle(builder::setVolume))
                .key("pitch", val -> val.floatValue().min(0).handle(builder::setPitch))
                .ifKey("break_sound", val -> val.string().map(ResourceLocation::new).handle(builder::setBreakSound))
                .ifKey("step_sound", val -> val.string().map(ResourceLocation::new).handle(builder::setStepSound))
                .ifKey("hit_sound", val -> val.string().map(ResourceLocation::new).handle(builder::setHitSound))
                .ifKey("fall_sound", val -> val.string().map(ResourceLocation::new).handle(builder::setFallSound))
        ;
